---
name: diagnostico-criativos
description: "Diagnostico de criativos com analise multimodal: matriz de avaliacao, padroes, analise de concorrentes e briefing de producao. Use quando o operador disser /diagnostico-criativos ou 'analisar criativos' ou 'avaliar anuncios' ou 'como estao os criativos'."
dependencies:
  - persona-icp
tools: []
week: 2
estimated_time: "2h"
output_file: "diagnostico-criativos.json"
multimodal: true
---
...
2. Leia `persona-icp.json` — extraia: RESUMO_ICP, linguagem do ICP, canais preferenciais
3. Se houver `diagnostico-midia.json`, carregue dados de performance dos criativos (CTR, CPL por criativo)
...
COPY — Diretrizes:
  ...
  Tom: {definido_no_posicionamento}
...
1. **Salve o JSON estruturado** em `clientes/{cliente}/diagnostico-criativos.json` seguindo o schema
2. **Atualize state.json** — marque `diagnostico-criativos` como `completed`
...
Este diagnostico alimenta:
  - /criativos-anuncios → Producao de novos criativos (Semana 3)
  - /copy-anuncios → Direcao de copy para os anuncios

Proximo passo recomendado: /diagnostico-cro
