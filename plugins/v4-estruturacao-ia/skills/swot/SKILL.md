...
2. `semana-1/diagnostico-maturidade.json` — scores e análise de maturidade (OBRIGATÓRIO — é dependência)
...
1. **Salve o output estruturado** em `clientes/{slug}/semana-1/swot.json` seguindo o schema.json da skill
2. **Registre a decisão** — appende em `decisions.jsonl`:
   ```json
   {"ts":"[ISO]","skill":"swot","checkpoint":3,"decision":"SWOT aprovada. Estratégia principal: [resumo em 1 frase]. Top ação: [ação #1]"}
   ```
3. **Atualize state.json** — marque `swot` como `completed`
4. **Informe próximos passos:**
   - "SWOT salva. Este output será usado pela skill posicionamento (semana 2) e como referência para todas as skills de produção."
   - Sugira a próxima skill da semana 1 (auditoria-comunicacao ou persona-icp se ainda não feita)
