---
name: crm-setup
description: "Configura o CRM Kommo com pipeline personalizado, réguas de boas-vindas e nutrição, e testa com lead fictício. Semi-manual — operador executa no Kommo. Use quando disser /crm-setup ou 'configurar CRM' ou 'setup Kommo' ou 'automação de leads'."
dependencies:
  - persona-icp
inputs:
  - briefing.json
...
2. `persona-icp.json` — ICP, dores, objeções, linguagem
3. `brandbook.json` — tom de voz, vocabulário (se existir — não é dependência obrigatória, mas melhora a qualidade)
...
- `{TOM_DE_VOZ}` — brandbook.verbal_identity (se existir) ou tom informado no briefing
...
Salve `crm-setup.json` completo com pipeline, réguas e checklist.
...
## Formato do output (crm-setup.json)
