...
2. Leia `persona-icp.json` — extraia: RESUMO_ICP, dores, comportamento, canais
3. Leia `diagnostico-comercial.json` — extraia: criterios de qualificacao 1-5 estrelas, objecoes mapeadas, SLA
4. Leia `brandbook.json` — extraia: TOM_DE_VOZ, personalidade da marca, vocabulario, palavras proibidas
5. Se houver `cliente-oculto.json`, leia os pontos criticos identificados
...
> Tom de voz: {TOM_DE_VOZ do brandbook}
...
Consulte `references/exemplos-scripts-sdr-whatsapp.md` para padroes de scripts naturais e exemplos de qualificacao.
...
ANALISE:
  ...
  Recomendacao: Opcao {X} porque {justificativa_baseada_no_brandbook}
```
...
REGRAS DE CONVERSA:
  ...
  - Se o lead fizer uma pergunta no meio, responda antes de continuar qualificando
  - Se o lead demonstrar objecao, trate antes de continuar (ver Checkpoint 4)
...
> O tom esta coerente com o brandbook?
...
Script de encaminhamento para vendedor:
  ...
  tem alguma duvida que posso tirar?"
...
> O tom esta consistente com o brandbook em todos os fluxos?
...
1. **Salve o JSON estruturado** em `clientes/{slug}/semana-5/scripts-sdr.json` seguindo o schema.json da skill
2. **Registre a decisao** — appende em `decisions.jsonl`:
   ```json
   {"ts":"[ISO]","skill":"scripts-sdr","checkpoint":5,"decision":"Scripts SDR IA aprovados. Agente: [nome]. X perguntas de qualificacao. Y objecoes tratadas. Pronto para configurar no Patagon."}
   ```
3. **Atualize state.json** — marque `scripts-sdr` como `completed`
4. **Informe proximos passos:**
   - "Scripts do SDR IA salvos. Proximo passo: `/sdr-ia-config` para configurar tudo no Patagon e integrar com Kommo."
