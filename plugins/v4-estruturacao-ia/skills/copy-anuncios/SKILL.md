...
2. `brandbook.json` — tom de voz, vocabulário, headlines aprovadas, CTAs, do's/don'ts
3. `persona-icp.json` — ICP, dores, desejos, objeções, linguagem
4. `posicionamento.json` — PUV, diferenciais, posicionamento
...
- `{TOM_DE_VOZ}` — brandbook.verbal_identity (resumo)
- `{PRODUTO_SERVICO}` — briefing.products_services
- `{OFERTA_CTA}` — oferta principal + CTA do brandbook
- `{PLATAFORMAS}` — plataformas confirmadas (Meta / Google / ambos)
- `{VOCABULARIO}` — palavras que usa/evita do brandbook
...
> 5. O tom é consistente com o brandbook?
>
...
Salve `copy-anuncios.json` completo com link da planilha.
