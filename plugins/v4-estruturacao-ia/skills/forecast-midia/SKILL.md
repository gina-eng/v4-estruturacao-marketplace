---
name: forecast-midia
description: "Cria o forecast de mídia de 3 meses: modelagem financeira, distribuição por plataforma/funil, cronograma e alertas. Output exportado para Google Sheets. Use quando disser /forecast-midia ou 'planejamento de mídia' ou 'forecast' ou 'budget de anúncios'."
dependencies:
  - diagnostico-midia
inputs:
  - briefing.json
  - diagnostico-midia.json
  - persona-icp.json
  - posicionamento.json
output: forecast-midia.json
...
2. `diagnostico-midia.json` — análise de mídia paga atual, CPL atual, ROAS atual, problemas identificados, benchmarks
3. `persona-icp.json` — ICP, canais preferidos
4. `posicionamento.json` — PUV, diferenciais
...
Salve `forecast-midia.json` completo com link da planilha.
...
## Formato do output (forecast-midia.json)
