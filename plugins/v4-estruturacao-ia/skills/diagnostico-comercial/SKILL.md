...
dependencies:
  - persona-icp
outputs: ["diagnostico-comercial.json"]
week: 4
estimated_time: "2h"
---
...
2. Leia `persona-icp.json` — extraia: RESUMO_ICP, dores principais, comportamento de compra, objecoes
...
Consulte `references/framework-diagnostico-comercial.md` para os benchmarks de conversao por segmento.
...
Com base nas objecoes listadas pelo operador e nas objecoes do ICP (persona-icp.json), gere:
...
REGRA DE OURO:
  Na duvida entre 3★ e 4★, classifique como {3★ ou 4★ — justifique a escolha}.
  Na duvida entre 2★ e 3★, classifique como {2★ ou 3★ — justifique}.
```
...
1. **Salve o JSON estruturado** em `clientes/{slug}/semana-4/diagnostico-comercial.json` seguindo o schema.json da skill
2. **Registre a decisao** — appende em `decisions.jsonl`:
   ```json
   {"ts":"[ISO]","skill":"diagnostico-comercial","checkpoint":4,"decision":"Diagnostico aprovado. Gargalo principal: [etapa]. Criterios 5★: [resumo]. SLA 5★: [X] min."}
   ```
3. **Atualize state.json** — marque `diagnostico-comercial` como `completed`
4. **Informe proximos passos:**
   - "Diagnostico comercial salvo. Este output sera usado por: cliente-oculto (simulacao de compra), scripts-sdr (scripts do agente IA), sdr-ia-config (configuracao do Patagon)."
   - Sugira a proxima skill: `/cliente-oculto` (para testar o processo atual antes de automatizar)
