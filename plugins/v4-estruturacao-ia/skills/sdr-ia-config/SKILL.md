---
name: sdr-ia-config
description: "Configuracao do SDR IA no Patagon + integracao Kommo: setup do agente, field mapping, alertas para vendedor e teste com 5 leads simulados. Use quando o operador disser 'configurar patagon', 'setup sdr', 'integrar kommo', 'colocar o agente no ar', ou apos scripts-sdr."
dependencies:
  - scripts-sdr
  - crm-setup
outputs: ["sdr-ia-config.json"]
week: 5
estimated_time: "4h"
semi_manual: true
---
...
> - Kommo CRM configurado com pipeline (skill crm-setup)
> - Scripts SDR aprovados pelo cliente (skill scripts-sdr)
> - Brandbook com tom de voz aprovado

## Setup

1. Leia `briefing.json` do cliente — extraia: NOME_CLIENTE, PRODUTO_SERVICO
2. Leia `scripts-sdr.json` — extraia: TODOS os scripts, perguntas, fluxos, objecoes
3. Leia `diagnostico-comercial.json` — extraia: criterios de qualificacao 1-5 estrelas, SLA
4. Leia `brandbook.json` — extraia: TOM_DE_VOZ, nome do agente
5. Verifique se `crm-setup.json` existe e esta completo (pipeline configurado no Kommo)
...
> Status atual: {status}
> Acao: execute `/scripts-sdr` (ou `/crm-setup`) antes de continuar.
...
>
> Se algum item for "nao", precisamos resolver antes de continuar.
...
PASSO 2: Configurar persona do agente
  ...
  - {regras_adicionais_do_brandbook}
  ---
...
PASSO 4: Configurar escalamento
  ...
  Se ninguem responder em {ESCALATION_MINUTES} minutos:
    ...
    e ja vai te chamar. Enquanto isso, tem alguma duvida que posso tirar?"
...
Para cada lead, verifique:
  ...
  [ ] Tom de voz consistente com brandbook
...
1. **Salve o JSON estruturado** em `clientes/{slug}/semana-5/sdr-ia-config.json` seguindo o schema.json da skill
2. **Registre a decisao** — appende em `decisions.jsonl`:
   ```json
   {"ts":"[ISO]","skill":"sdr-ia-config","checkpoint":4,"decision":"SDR IA configurado e testado. 5/5 testes passaram. Agente: [nome]. Integracao Kommo ativa. Go-live aprovado."}
   ```
3. **Atualize state.json** — marque `sdr-ia-config` como `completed`
