---
name: auditoria-comunicacao
description: "Audita todos os pontos de contato digitais do cliente (site, Instagram, anúncios, GMB, WhatsApp) e gera matriz de gaps com quick wins. Usa capacidade multimodal para analisar screenshots. Use quando o operador disser 'auditoria', 'comunicação', 'pontos de contato', 'gaps', ou após completar persona-icp."
dependencies: ["persona-icp"]
outputs: ["auditoria-comunicacao.json"]
week: 1
...
2. `semana-1/persona-icp.json` — ICP e persona (OBRIGATÓRIO — é dependência)
...
1. Mostre ao operador o que já sabe:
   ...
   ICP REFERÊNCIA: [resumo da persona-icp]
...
- Está alinhada com a mensagem-chave aprovada na skill persona-icp?
...
**Coerência visual:**
- Visual condiz com o posicionamento da marca?
...
1. **Salve o output estruturado** em `clientes/{slug}/semana-1/auditoria-comunicacao.json` seguindo o schema.json da skill
2. **Registre a decisão** — appende em `decisions.jsonl`:
   ```json
   {"ts":"[ISO]","skill":"auditoria-comunicacao","checkpoint":4,"decision":"Auditoria aprovada. [X] canais auditados. Top gap: [gap]. Top quick win: [win]"}
   ```
3. **Atualize state.json** — marque `auditoria-comunicacao` como `completed`
4. **Informe próximos passos:**
   - "Auditoria salva. Os gaps identificados serão endereçados nas skills de produção (semana 3): landing-page, identidade-visual, brandbook."
