---
name: diagnostico-cro
description: "Diagnostico de CRO (Conversion Rate Optimization): analise tecnica, auditoria de copy, hipoteses de teste e wireframe de melhorias para a landing page. Use quando o operador disser /diagnostico-cro ou 'analisar conversao' ou 'diagnostico da landing page' ou 'por que a LP nao converte' ou 'analise de CRO'."
dependencies:
  - persona-icp
  - posicionamento
tools: []
week: 2
estimated_time: "2.5h"
output_file: "diagnostico-cro.json"
multimodal: true
---
...
2. Leia `persona-icp.json` — extraia: RESUMO_ICP, dores, linguagem, canal preferencial
3. Leia `posicionamento.json` — extraia: PUV, mensagem topo/fundo de funil, tom de comunicacao
4. Se houver `diagnostico-midia.json`, carregue taxa de conversao e bounce rate
...
Com base em toda a analise, gere o wireframe da LP melhorada que sera input direto para a skill `/landing-page` da Semana 3.
...
1. **Salve o JSON estruturado** em `clientes/{cliente}/diagnostico-cro.json` seguindo o schema
2. **Atualize state.json** — marque `diagnostico-cro` como `completed`
...
Este diagnostico alimenta DIRETAMENTE:
  - /landing-page → Copy, estrutura e codigo da nova LP (Semana 3)

Com a analise de CRO + posicionamento + persona, a LP da Semana 3
tera direcao precisa de conteudo e estrutura.

SEMANA 2 COMPLETA!
Skills concluidas: pesquisa-mercado, posicionamento, diagnostico-midia,
diagnostico-criativos, diagnostico-cro

Proximo passo: Semana 3 — Producao
Comece por: /identidade-visual ou /brandbook
```
