---
name: posicionamento
description: "Canvas de posicionamento estrategico completo: PUV, 4Ps, territorio de marca e taglines. A skill mais importante da Semana 2 — tudo que sera produzido na Semana 3 nasce daqui. Use quando o operador disser /posicionamento ou 'definir posicionamento' ou 'PUV' ou 'proposta de valor' ou 'canvas de posicionamento'."
dependencies:
  - pesquisa-mercado
  - persona-icp
  - swot
...
week: 2
estimated_time: "2.5h"
output_file: "posicionamento.json"
---
...
2. Leia `persona-icp.json` — extraia: RESUMO_ICP, dores, desejos, linguagem, Jobs-to-be-Done
3. Leia `pesquisa-mercado.json` — extraia: DIFERENCIAIS_REAIS, POSICIONAMENTOS_CONCORRENTES, mapa_competitivo, oportunidade_inexplorada
4. Leia `swot.json` — extraia: RESUMO_SWOT (forcas + oportunidades prioritarias)
...
Concorrentes e seus posicionamentos:
  - {concorrente_1}: {posicionamento}
  - {concorrente_2}: {posicionamento}
  - {concorrente_3}: {posicionamento}
...
Antes de gerar qualquer posicionamento, confirme com o operador a direcao estrategica.
...
>
> 3. **Restricao de posicionamento:** Existe algo que o cliente NAO quer ser associado? (ex: "nao quero ser visto como barato", "nao quero ser comparado com X")
>
...
## Checkpoint 2 — Mapa de posicionamento competitivo + 3 opcoes de PUV

Com base nos inputs e nas respostas do checkpoint 1, gere o mapa de posicionamento e 3 opcoes de direcao.
...
### 3 Declaracoes de posicionamento
...
Com o posicionamento e canvas validados, defina o territorio de marca e crie opcoes de tagline.
...
> Este e o posicionamento que vai guiar TUDO da Semana 3: brandbook, landing page, criativos, copy de anuncios. Esta consistente e aprovado?
>
...
1. **Salve o JSON estruturado** em `clientes/{cliente}/posicionamento.json` seguindo o schema
2. **Atualize state.json** — marque `posicionamento` como `completed`
...
Este posicionamento sera usado por:
  - /brandbook → manual de copy e tom de voz
  - /landing-page → copy e estrutura da pagina
  - /copy-anuncios → mensagens de anuncio por funil
  - /criativos-anuncios → briefing criativo
  - /identidade-visual → direcionamento de design

Proximo passo recomendado: /diagnostico-midia ou /diagnostico-criativos
