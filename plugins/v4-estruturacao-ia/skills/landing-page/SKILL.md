---
name: landing-page
description: "Cria a landing page de conversão: copy completa seção por seção, geração de código React+Tailwind, e deploy na Vercel. Mix auto + manual. Use quando disser /landing-page ou 'criar LP' ou 'landing page' ou 'página de conversão'."
dependencies:
  - posicionamento
  - brandbook
  - diagnostico-cro
...
2. `posicionamento.json` — PUV, posicionamento, diferenciais
3. `brandbook.json` — tom de voz, paleta, tipografia, vocabulário, headlines aprovadas, CTAs
4. `diagnostico-cro.json` — análise de conversão, problemas identificados no site atual, wireframe sugerido
5. `identidade-visual.json` — se existir, usar paleta e tipografia de lá
...
- `{TOM_DE_VOZ}` — do brandbook.verbal_identity
- `{OFERTA_CTA}` — CTA principal do brandbook
- `{WHATSAPP}` — briefing.whatsapp (formato: 5511999999999)
- `{PALETA}` — cores do brandbook ou identidade-visual
- `{TIPOGRAFIA}` — fontes do brandbook ou identidade-visual
...
Salve `landing-page.json` completo com URL de deploy.
...
## Formato do output (landing-page.json)
