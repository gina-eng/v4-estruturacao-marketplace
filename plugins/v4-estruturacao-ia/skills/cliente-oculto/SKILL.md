...
2. Leia `persona-icp.json` — extraia: RESUMO_ICP, perfil demografico, comportamento de compra
3. Leia `diagnostico-comercial.json` — extraia: objecoes mapeadas, gargalos do funil, SLA definido
...
Consulte `references/criterios-avaliacao-cliente-oculto.md` para os criterios que serao usados na avaliacao.
...
Consulte `references/criterios-avaliacao-cliente-oculto.md` para aplicar os criterios de avaliacao.
...
1. **Salve o JSON estruturado** em `clientes/{slug}/semana-4/cliente-oculto.json` seguindo o schema.json da skill
2. **Registre a decisao** — appende em `decisions.jsonl`:
   ```json
   {"ts":"[ISO]","skill":"cliente-oculto","checkpoint":3,"decision":"Cliente oculto executado. Nota geral: [X]/10. Pontos criticos: [lista]. Impacto SDR IA estimado: +R$[valor]/mes."}
   ```
3. **Atualize state.json** — marque `cliente-oculto` como `completed`
4. **Informe proximos passos:**
   ...
   - Sugira a proxima skill: `/scripts-sdr` (para criar os scripts que corrigem os problemas encontrados)
