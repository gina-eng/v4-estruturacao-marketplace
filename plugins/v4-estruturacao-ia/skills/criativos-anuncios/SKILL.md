...
description: "Cria o briefing criativo para anúncios: 5 variações com hooks diferentes, prompts Midjourney/Ideogram e organização do pack. Semi-manual — operador gera imagens externamente. Use quando disser /criativos-anuncios ou 'criativos de ads' ou 'pack de anúncios' ou 'imagens para anúncio'."
dependencies:
  - brandbook
  - identidade-visual
  - diagnostico-criativos
inputs:
  - briefing.json
  - brandbook.json
  - identidade-visual.json
  - diagnostico-criativos.json
output: criativos-anuncios.json
week: 3
type: semi-manual
estimated_time: "4h"
---
...
2. `brandbook.json` — tom de voz, vocabulário, headlines, paleta (se identidade-visual não existir)
3. `identidade-visual.json` — paleta de cores, tipografia, estilo visual, conceito criativo
4. `diagnostico-criativos.json` — análise dos criativos atuais, o que funciona/não funciona, recomendações
...
- `{CTA}` — CTA principal
- `{TOM_DE_VOZ}` — brandbook.verbal_identity
...
> 3. O tom está alinhado com o brandbook?
...
Salve `criativos-anuncios.json` completo.
...
## Formato do output (criativos-anuncios.json)
