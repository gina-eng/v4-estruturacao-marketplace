---
name: brandbook
description: "Cria o Brandbook completo: propósito, identidade verbal, narrativa da marca e banco de copy. Totalmente automatizado com checkpoints de validação. Use quando disser /brandbook ou 'manual de marca' ou 'tom de voz' ou 'banco de copy'."
dependencies:
  - posicionamento
  - persona-icp
inputs:
...
2. `posicionamento.json` — PUV, posicionamento, território de marca, tom de voz, diferenciais
3. `persona-icp.json` — ICP, persona, Jobs-to-be-Done, dores, objeções
...
- `{DIFERENCIAIS}` — posicionamento.differentials
...
Crie a seção de propósito e posicionamento da marca. Consulte `references/exemplos-brandbook.md` para referências de bom vs. ruim.
...
1 frase de posicionamento no formato: "Para [ICP] que [necessidade], {NOME_CLIENTE} é [categoria] que [diferencial] porque [razão para acreditar]."
...
5-7 frases que sintetizam o posicionamento. Devem funcionar como taglines, assinaturas de post, ou headers de material.
...
Salve `brandbook.json` completo com todas as seções.
...
## Formato do output (brandbook.json)
