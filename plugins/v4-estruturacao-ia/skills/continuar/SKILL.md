...
2. Se nenhum cliente existe: diga "Nenhum cliente cadastrado. Quer cadastrar? Diga /novo-cliente" e encerre
...
Ex: "continuar Padaria Silva" — resolva o slug (lowercase, hifenizado, sem acento) e tente encontrar `clientes/{slug}/state.json`. Se encontrou, carregue direto sem mostrar panorama. Se não encontrou, faça busca parcial no nome dos clientes disponíveis e sugira.
...
- Semana 1: diagnostico-maturidade, persona-icp, swot, auditoria-comunicacao
- Semana 2: pesquisa-mercado, posicionamento, diagnostico-midia, diagnostico-criativos, diagnostico-cro
- Semana 3: identidade-visual, brandbook, landing-page, copy-anuncios, criativos-anuncios, crm-setup, forecast-midia, gmb-otimizacao
- Semana 4-5: diagnostico-comercial, cliente-oculto, scripts-sdr, sdr-ia-config
...
Em andamento:
  auditoria-comunicacao (checkpoint 2/4)

Próximas disponíveis:
  pesquisa-mercado (dependências OK)
  diagnostico-midia (dependências OK)

Bloqueadas:
  posicionamento (falta: pesquisa-mercado)

Última decisão: "Tom mais informal, foco em donas de casa 35-50" (06/04)

Recomendo continuar auditoria-comunicacao (checkpoint 2). Quer seguir ou prefere outra skill?
```
...
3. Verifique outputs de skills dependentes:
   - Leia o campo `summary` de cada output JSON das dependências (ex: `persona-icp.json` → campo `summary`)
